...
public:
    ...
    popen(const std::string& cmd, std::vector<std::string> argv, std::vector<std::string> env)
    ...
    popen(const std::string& cmd, std::vector<std::string> argv, std::vector<std::string> env, std::ostream& pipe_stdout)
        : in_filebuf(nullptr), out_filebuf(nullptr), err_filebuf(nullptr), in_stream(nullptr), out_stream(nullptr), err_stream(nullptr)
    {
...
private:
    ...
    void run(const std::string& cmd, std::vector<std::string> argv, std::vector<std::string> env)
    {
        argv.insert(argv.begin(), cmd);

        pid = ::fork();
      
        if (pid == 0) child(argv, env);
    ...
    void child(const std::vector<std::string>& argv, const std::vector<std::string>& env)
    {  
        ...
        std::vector<raii_char_str> real_env(env.begin(), env.end());
        std::vector<char*> cenv(real_env.begin(), real_env.end());
        cenv.push_back(nullptr);
        
        if (execvpe(cargs[0], &cargs[0], &cenv[0]) == -1)
        {
